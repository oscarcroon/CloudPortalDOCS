...
// Also emit an importable TS module so the docs site can resolve locale paths
// synchronously at build time (SSR hreflang + client switcher) — a runtime
// fetch of docs-map.json doesn't resolve reliably during prerender.
const TS_OUT = join(ROOT, 'app', 'docs-map.generated.ts')
await mkdir(join(ROOT, 'app'), { recursive: true })
await writeFile(
  TS_OUT,
  '// AUTO-GENERATED by scripts/gen-docs-map.mjs — do not edit.\n'
  + 'export interface DocsPagePair { key: string, sv: string, en: string }\n'
  + `export const docsPages: DocsPagePair[] = ${JSON.stringify(pagePairs, null, 2)}\n`,
  'utf8'
)

console.log(`✅ Wrote ${relative(ROOT, OUT)} + ${relative(ROOT, TS_OUT)} (${routes.length} routes, ${pagePairs.length} page pairs)`)
